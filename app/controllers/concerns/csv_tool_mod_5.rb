...
module CsvToolMod
  ...
  def uni_account_migrator
    ...
    all_account_ids = UniAccount.pluck(:id)
    uniq_account_ids = UniAccount.distinct(:crm_acct_num).pluck(:id)
    @duplicate_account_ids = all_account_ids - uniq_account_ids

    accounts = []

    # uniq_account_ids[0..20].each do |uni_account_id|
    uniq_account_ids.each do |uni_account_id|
      uni_account = UniAccount.find(uni_account_id)
      ...
      begin
        ...
        if account.present?
          update_obj_if_changed(account_hash, account)
        else
    ...
    uniq_account_ids.map(&:delete)
    all_account_ids.map(&:delete)
    binding.pry

    if !@duplicate_account_ids.empty?
      binding.pry
      @duplicate_account_ids
      ### Will need to iterate through this batch if not nill.
    end

    #### ABOVE IS ACCOUNTS, BELOW IS PHONES ####
